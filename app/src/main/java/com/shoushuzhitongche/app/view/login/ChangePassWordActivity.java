...
public class ChangePassWordActivity extends BaseActivity{
	...
	public void doForgetPsw(View view){
		...
		if(pwdStr.length()<6){
			ToastUtil.showToast(activity, "密码长度至少6位！");
			return;
		}
		
		Map map = new HashMap<String,Object>();
		map.put("username", phoneStr);
		map.put("password_new", pwdStr);
		map.put("verify_code", codeStr);
		map.put("key", "forgetpassword");
		LoadingDialog.getInstance(activity).show();
		String url = CommonUtils.getForgetpassword(activity);
