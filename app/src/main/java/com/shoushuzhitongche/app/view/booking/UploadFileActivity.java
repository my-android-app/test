...
public class UploadFileActivity extends BaseActivity {
	...
	private int sum = 0;
	private boolean isReturn;//true 说明 该步操作来自专家预约，返回时，需要刷新数据  ； false 执行正常流程
	...
	private Handler handler = new Handler(){
		...
		public void handleMessage(Message msg) {
			...
			if(msg.what == 1){
				...
				if(!isReturn) button.setText("保存");
				else button.setText("下一步");
			}
			if(msg.what == 0){
				sum++;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.activity_create_upload_file);
		initView();
		initData();
	}
	...
	private void initData() {
		...
		id = getIntent().getStringExtra(Constants.PARMS.ID);
		button = (Button)findViewById(R.id.btn_commit);
		isReturn = getIntent().getBooleanExtra(Constants.PARMS.IS_NEED_RETURN, false);
		if(isReturn){
			button.setText("保存");
		}
	}
	...
	public void initHeaderView() {
		...
		initOptionView(0, R.string.option_skip, new OnClickListener() {
			@Override
	...
	private void returnResult(){
        ...
        intent.putExtra(Constants.PARMS.BACK_DATA,id);  
	...
	private void upQiniu() {
		...
		UploadUtils uploadUtils =new UploadUtils( id);//现在随便写了一个
		///医生端上传患者预约病历或小结的空间权限
...
//			map.put("appFile[username]", "13127640379");
			...
			map.put("appFile[patientId]", id);//现在随便写了一个患者id
	...
	private void goToIntentionActivity(){
		if(!isReturn){
