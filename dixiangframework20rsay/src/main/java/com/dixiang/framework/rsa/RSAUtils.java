package com.dixiang.framework.rsa;
...
public final class RSAUtils {
	...
	// web和android默认补位方式不一样
	// private static String RSA_ANDROID = "RSA/ECB/PKCS1Padding";
	private static String RSA_WEB = "RSA/None/PKCS1Padding";
	...
	public static byte[] encryptData(byte[] data) {
		try {
			Cipher cipher = Cipher.getInstance(RSA_WEB);
			// 编码前设定编码方式及密钥
			cipher.init(Cipher.ENCRYPT_MODE, SystemConstants.publicKey);
			// 传入编码数据并返回编码结果
	...
	public static byte[] decryptData(byte[] encryptedData) {
		try {
			Cipher cipher = Cipher.getInstance(RSA_WEB);
			cipher.init(Cipher.DECRYPT_MODE, SystemConstants.privateKey);
